...
export function CourseDescription({ text }: { text: string }) {
  ...
  useEffect(() => {
    ...
    const check = () => {
      // scrollHeight is unreliable with -webkit-line-clamp (the -webkit-box
      // display model reports scrollHeight === clientHeight regardless of
      // overflow). Instead, compare the actual clientHeight against the
      // theoretical max height of 4 lines: when the element fills all 4 lines
      // the text is either exactly 4 lines or overflows → show the button.
