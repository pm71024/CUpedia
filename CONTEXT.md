# CUpedia 权限与用户管理

CUpedia 的用户身份与访问控制术语。本文件只是术语表，不记录实现。

## Language

**User（普通用户）**:
通过 CUHK 邮箱注册的账号，默认角色。
_Avoid_: Account、Member

**Admin（管理员）**:
`role === "admin"` 的 User。拥有除"角色管理"外的全部后台权限：封禁/解封普通 User、删除/恢复页面、切换 Editor mode、管理讨论。**不能**任命或撤销管理员，也不能封禁/降级 站长——那是 站长 的专属权。由 站长 在后台任命产生。
_Avoid_: Superuser

**站长（Owner）**:
全站**恰好一个**的最高权限持有者，是一名被指定的 Admin——`role` 仍为 `"admin"`，**不是**独立的 role 取值，而是 Admin 的超集。唯一能变更他人 role（任命/撤销 Admin）的身份。作为信任根，站长**不可被任何 Admin 封禁或降级**，只能通过直接改数据库来变更——这是它与普通 Admin 的根本区别。
_Avoid_: Superuser、超管（口语可，规范词用 站长 / Owner）；不要把 站长 当成第三个 role 字符串

**Editor mode（编辑模式）**:
站点级开关 `wiki_edit_role ∈ {admin, user}`，决定"谁能编辑 wiki"。它是**模式**，不是**角色**——系统中不存在 per-user 的"编辑者"角色。
_Avoid_: 把"编辑者/Editor"当作一种用户角色来谈

**Banned（封禁）**:
User 上的布尔标记。写路径每次查库即时生效；读路径（session cookie 缓存）最多滞后 5 分钟。
_Avoid_: Disabled、Suspended

**Deleted page（已删除页面）**:
`deletedAt` 非空的 wiki 页面。采用 Wikipedia 模型——**存活**页面的历史/diff 公开可读；**已删除**页面的列表、正文、历史均仅限 Admin。普通 User 对已删页面只应看到"已删除"状态，看不到内容。例外：该页**上传的配图**有独立生命周期，不随页面删除而撤下（见 ADR 0002）。
_Avoid_: 把软删除当作纯导航隐藏；把“删页”理解为连带抹除媒体

**Page ID（页面标识）**:
Wiki page 的数据库 UUID，也是 `/wiki/<page-id>` 的永久公开地址和未来协作文档键。标题、父页面、图标均可变，不构成身份；项目没有 slug、alias 或改名重定向。
_Avoid_: Slug、页面路径、用标题指代稳定身份

**Private page draft（私有页面草稿）**:
新 Wiki 页面发布前的所有者专属状态。它拥有稳定 Page ID，但不属于公开 Wiki；发布后，同一 Page ID 才成为公开页面。只有所有者可以读取或修改。
_Avoid_: 公开空白页、未发布的公开页面、Local draft

**Local draft（本地草稿）**:
某个 User 在某个 Page ID 和浏览器编辑会话中尚未获服务器确认的恢复副本。它不改变页面可见性，也不代表离线编辑；公开页面或 Private page draft 才是服务器权威版本。
_Avoid_: Private page draft、offline page、把本地草稿当作协作状态

**Discussion（讨论）**:
依附在 wiki 页面某段文字上的行内评论及其回复线程。任何登录 User 均可发起（与 Editor mode 无关，等同 Wikipedia talk page——锁定正文编辑也不锁讨论）；删除/标记解决限本人或 Admin。
_Avoid_: Comment（口语可，但代码/表名以 Discussion 为准）

**Eligible account（合规账号）**:
唯一允许注册/登录的身份：`@cuhk.edu.hk`，或 `@link.cuhk.edu.hk` 且前缀形如 `1155xxxxxx`。这是"谁能拥有账号"的授权边界，必须在**服务端发码环节**强制，而非仅客户端。
_Avoid_: 把白名单只放在 UI 层

## Read access（读边界）

公开阅读模型（Wikipedia）：wiki 正文与图片均对匿名开放，登录仅用于编辑/评论。注意这与"注册需 CUHK 邮箱"是有意分离的——**读公开、写/账号受限**。

## Flagged ambiguities

- **"编辑者" ≠ 角色**：当有人说"编辑者能不能删页面"时，先确认指的是"处于编辑模式下的 User"，而非某个独立角色。本项目有 User / Admin / 站长 三级，其中 站长 是 Admin 的超集（同一个 `role: "admin"`，是被另行指定的那一个 Admin），并非第三个 role 字符串。

## 示例对话

> **甲**：编辑者能删页面吗？
> **乙**：项目里没有"编辑者"角色。你是说"开放编辑模式下的普通 User"吗？那种 User 能创建和编辑，但删除/恢复只限 Admin。
> **甲**：那 Admin 怎么来的？
> **乙**：由 站长在后台任命。站长是全站唯一的最高权限者，作为信任根只能改库产生；普通 Admin 之间无权互相任免，也动不了站长。
