...
- **主修要求骨架**（类目 + 每类应修学分 + 成员课号）—— Handbook 现已把 Major requirements 跳转到 AQS `Browse Program Information`；按当前 Handbook 年份查询其官方 study scheme。
...
- AQS Program Information 搜索有 CAPTCHA；按当前 Handbook 标题确定最新届，并抓最近四个入学年份。必须从 scheme 正文识别 Major，不能把 Minor/GE/Faculty Package 写入主修表。
...
- **抓取（隔离）**：`tools/scraper/`——独立 venv、不入 pnpm/CI/runtime。`scrape_courses.py`（科目枚举 + ddddocr 过验证码 + 详情页）→ `scripts/data/courses.json`；`scrape_handbook.py`（Program Information 按最近四届查询 + Major 分类 + 可恢复 manifest）→ `scripts/data/handbook/`。harvest 产物 gitignore，不提交。
